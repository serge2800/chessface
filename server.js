...
function soundUrl(relativePath) {
  return `/assets/sounds/${relativePath.split(path.sep).map(encodeURIComponent).join("/")}`;
}

function readSoundManifest() {
  ...
  function walk(directory, parts = []) {
    fs.readdirSync(directory, { withFileTypes: true })
      ...
      .forEach((entry) => {
        ...
        const baseId = `${soundSlug(parts[0] || "sounds")}-${soundSlug(entry.name)}`;
        const id = ids.has(baseId) || baseId.length > 72 ? `${soundSlug(entry.name)}-${hash}` : baseId;
        ids.add(id);
        items.push({
          id,
          label: soundLabel(entry.name),
          category: parts[0] || "Sounds",
          file: soundUrl(relativePath)
        });
      });
  }

  walk(SOUND_SOURCE_DIR);
  return items;
}
...
app.get("/api/sounds", (_req, res) => {
  res.json({
    source: fs.existsSync(SOUND_SOURCE_DIR) ? SOUND_SOURCE_DIR : null,
    sounds: readSoundManifest()
  });
});
...
function gamePayload(game, viewerId) {
  ...
  return {
    ...
    canUseRandomSound: canUseRandomTurnSound(game, viewerId),
    randomSoundLocked: isRandomSoundLocked(game),
...
function takebackPayload(game, viewerId) {
  const moveCount = game.chess.history().length;
  const used = game.takebackUsed?.[viewerId] || 0;
  const remaining = Math.max(0, TAKEBACK_REQUEST_LIMIT - used);
  const request = game.takebackRequest || null;
  const pendingRequest = request && request.status !== "expired";
  const color = colorForUser(game, viewerId);
  const turnColor = game.chess.turn() === "w" ? "white" : "black";
  return {
    limit: TAKEBACK_REQUEST_LIMIT,
    used,
    remaining,
    requestedThisMove: game.takebackRequestedMove?.[viewerId] === moveCount,
    canRequest: Boolean(
      game.kind !== "team"
      && game.status === "playing"
      && color
      ...
      && remaining > 0
      && !pendingRequest
      && game.takebackRequestedMove?.[viewerId] !== moveCount
    ),
    request: request ? {
      id: request.id,
      ...
      isRequester: request.requesterId === viewerId,
      isReceiver: request.receiverId === viewerId
    } : null
  };
}
...
function normalizeSoundSettings(settings = {}) {
  ...
  const checkmateSound = validSoundId(settings.checkmateSound) ? settings.checkmateSound : "random";
...
function activeTurnPlayer(game) {
  ...
  if (game.kind === "team") return currentTeamMover(game);
  return color === "white" ? game.white : game.black;
}

function randomSoundTurnKey(game, playerId) {
  return `${game.chess.history().length}:${playerId}`;
}

function isRandomSoundLocked(game) {
  return Boolean(game?.soundLockedUntil && Date.now() < game.soundLockedUntil);
}

function randomPlayableSoundId() {
  const sounds = readSoundManifest().filter((sound) => sound.file && sound.id !== "random" && sound.id !== "none");
  if (!sounds.length) return "random";
...
function scheduleRandomSoundUnlock(game) {
  ...
  const timer = setTimeout(() => {
    soundUnlockTimers.delete(game.id);
    if (games.get(game.id) !== game || game.status !== "playing") return;
    if (isRandomSoundLocked(game)) return scheduleRandomSoundUnlock(game);
    game.soundLockedUntil = 0;
    emitGame(game);
  }, delay + 40);
  soundUnlockTimers.set(game.id, timer);
}

function canUseRandomTurnSound(game, playerId) {
  if (!game || game.status !== "playing" || isRandomSoundLocked(game)) return false;
  const color = colorForUser(game, playerId);
  const turnColor = game.chess.turn() === "w" ? "white" : "black";
  if (!color || color !== turnColor) return false;
  if (game.kind === "team") {
    const activePlayer = currentTeamMover(game);
...
io.on("connection", (socket) => {
  ...
  socket.on("game:move", ({ from, to, promotion }) => {
    ...
    let move = null;
    try {
      const promotionPiece = typeof promotion === "string" && /^[qrbn]$/.test(promotion) ? promotion : undefined;
      const needsPromotion = game.chess.moves({ verbose: true }).some((legalMove) => (
        legalMove.from === from && legalMove.to === to && legalMove.promotion
      ));
      if (needsPromotion && !promotionPiece) throw new Error("Promotion piece required.");
      move = game.chess.move({ from, to, ...(promotionPiece ? { promotion: promotionPiece } : {}) });
    } catch {
      move = null;
    }
    if (!move) {
    ...
    if (game.takebackRequest?.status === "begging") {
      game.takebackRequest.status = "expired";
      game.takebackRequest.expiredAt = Date.now();
    } else {
  ...
  socket.on("game:takeback:request", () => {
    const state = sockets.get(socket.id);
    const game = games.get(state?.gameId);
    if (!game || game.status !== "playing" || game.kind === "team") return;
    settleGameClock(game);
    if (game.status !== "playing") return;
    const color = colorForUser(game, socket.user.id);
    const turnColor = game.chess.turn() === "w" ? "white" : "black";
    const moveCount = game.chess.history().length;
    if (!color || color === turnColor || moveCount === 0) return;
    game.takebackUsed ||= {};
    game.takebackRequestedMove ||= {};
    const used = game.takebackUsed[socket.user.id] || 0;
    if (used >= TAKEBACK_REQUEST_LIMIT) {
      return socket.emit("takeback:notice", { message: "You ran out of takeback requests", board: true });
    }
    if (game.takebackRequest) return socket.emit("error:message", "A takeback request is already pending.");
    if (game.takebackRequestedMove[socket.user.id] === moveCount) {
      return socket.emit("error:message", "You already requested a takeback for this move.");
    }
    const lastMove = game.chess.history({ verbose: true }).at(-1);
    if (!lastMove || (lastMove.color === "w" ? "white" : "black") !== color) return;
    const receiver = color === "white" ? game.black : game.white;
    ...
    game.takebackRequestedMove[socket.user.id] = moveCount;
    emitGame(game);
  });

  socket.on("game:takeback:respond", ({ action }) => {
    const state = sockets.get(socket.id);
    const game = games.get(state?.gameId);
    if (!game || game.status !== "playing" || game.kind === "team" || !game.takebackRequest) return;
    const request = game.takebackRequest;
    if (request.status === "expired") return;
    if (request.receiverId !== socket.user.id) return;
    const response = String(action || "").toLowerCase();
    if (!["accept", "reject", "beg"].includes(response)) return;

    if (response === "beg") {
      request.status = "begging";
      const requester = gamePlayers(game).find((player) => player.id === request.requesterId);
      if (requester) io.to(requester.socketId).emit("takeback:notice", { message: "YOU MAY BEG NOW", board: true, duration: 2000 });
      emitGame(game);
      return;
    }

    game.takebackUsed ||= {};
    game.takebackUsed[request.requesterId] = Math.min(TAKEBACK_REQUEST_LIMIT, (game.takebackUsed[request.requesterId] || 0) + 1);

    if (response === "reject") {
      const requester = gamePlayers(game).find((player) => player.id === request.requesterId);
      game.takebackRequest = null;
      if (requester) io.to(requester.socketId).emit("takeback:notice", { message: "Your takeback request was rejected", board: true });
      emitGame(game);
      return;
    }

    if (game.chess.history().length !== request.moveCount) {
      game.takebackRequest = null;
      socket.emit("error:message", "That takeback request is no longer available.");
      emitGame(game);
      return;
    }
    const lastMove = game.chess.history({ verbose: true }).at(-1);
    if (!lastMove || (lastMove.color === "w" ? "white" : "black") !== request.requesterColor) {
      game.takebackRequest = null;
      socket.emit("error:message", "That takeback request is no longer available.");
      emitGame(game);
      return;
    ...
    settleGameClock(game);
    game.chess.undo();
    game.clocks[request.requesterColor] = Math.max(0, game.clocks[request.requesterColor] - game.increment);
    game.drawOfferFrom = null;
    game.takebackRequest = null;
  ...
  socket.on("sound:random-turn", () => {
    ...
    if (isRandomSoundLocked(game)) {
      return socket.emit("error:message", "Wait for the current sound to finish.");
    }
    if (!canUseRandomTurnSound(game, socket.user.id)) {
      return socket.emit("error:message", "You already used your random sound this turn.");
    }
    game.randomSoundPushes ||= new Set();
